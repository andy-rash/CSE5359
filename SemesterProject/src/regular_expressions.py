...
The NANP regular expression was partially derived from a Stack Overflow response
here:
	https://stackoverflow.com/a/123666

It accepts only 10 digit phone numbers but allows highly variable formatting
styles since the input gets sanitized ahead of time.


The E.164 regular expression has been compiled using all known country calling
codes in addition to the ITU E.164 standard for lengths.

Country calling codes were scraped from Wikipedia using a custom Python script.
This script is located at `SemesterProject/utils/wiki-scraper.py`.
